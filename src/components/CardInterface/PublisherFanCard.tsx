...
interface PublisherFanCardProps {
  device: MqttDeviceConfig;
  setDetail?: (device: MqttDeviceConfig) => void;
}

export default function PublisherFanCard({ device, setDetail }: PublisherFanCardProps) {
  ...
  const handleSwitchClick = (e: React.MouseEvent) => {
    ...
    console.log(`Status kipas: ${message}`);

    if (isConnected) {
      publish(message); // Kirim pesan ON/OFF ke topik device
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Kipas Animasi */}
        <div
          className={`w-8 h-8 border-4 rounded-full relative transition-transform duration-500 ${
            isOn
              ? "animate-spin border-t-blue-400 border-r-blue-400 border-b-gray-300 border-l-gray-300"
              : "border-gray-400"
          }`}
        ></div>
